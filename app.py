...
def render_sources(sources):
    ...
    with st.expander("📎 Verified sources"):
        ...
        for i, src in enumerate(sources):
            ...
            # Use safe .get() defaults so old session history never crashes
            doc_type = src.get('doc_type', 'report').upper()
            year = src.get('year', '2026')
            page_num = src.get('page_num', 'N/A')
            stamps_html += f"""
            <div class="stamp" style="transform:rotate({rotate});">
              <div class="stamp-head">✓ VERIFIED · {doc_type} · {year} · PG {page_num}</div>
            </div>
            """
        stamps_html += "</div>"
